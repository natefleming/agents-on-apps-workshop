...
def get_databricks_host_from_env() -> Optional[str]:
    """Get the Databricks host URL from environment/SDK config."""
    try:
        w = WorkspaceClient()
...
async def process_agent_astream_events(
    ...
    This processes two types of events from agent.astream():
    - "updates": Complete tool call results and final messages
    - "messages": Streaming text chunks (token by token)
    """
